...
- Season-to-date views: every team is ranked and percentiled from its first game, and the EPA value always shows. Below **30** cumulative plays-in-phase the row is flagged and rendered with a small "n < 30" badge beside the rank as a caution; nothing is withheld. All 32 teams share the same early-season sample, so the numbers are honest even though a rank will move week to week (this is what Sumer Sports, rbsdm and FTN do). A team leaves the 1–32 denominator only when its metric cannot be computed (e.g. a one-sided `overall` differential). Multi-season trend charts still leave a sub-floor season unplotted rather than drawing a point that will move.
